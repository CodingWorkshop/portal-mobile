...
const SET_THEME = 'theme-' + ((ALL_THEME.some(i => i == input)) ? input : 'light');
...
function BuildLocalTheme(baseTheme, setTheme) {
    ...
    return baseTheme[0] + setTheme + baseTheme[1] + 'theme' + baseTheme[2] + setTheme + baseTheme[3]
}
...
function applyLocalTheme(setTheme) {
    fs.readFile('src/style/themes/_set-theme.scss', 'utf8')
        ...
        .then(baseTheme =>
            fs.writeFile('src/style/themes/build-theme.scss', BuildLocalTheme(baseTheme, setTheme), 'utf8')
        )
...
function applyCustomizeTheme(httpTheme) {
    fs.readFile('src/style/themes/_set-customize-theme.scss', 'utf8', (e, baseTheme) => {
        baseTheme = baseTheme.split('//customize-color');
        request
            .get(httpTheme, (error, response, color) => {
                baseTheme = baseTheme[0] + JSON.parse(color).theme + baseTheme[1];
                fs.writeFile('src/style/themes/_theme-customize.scss', baseTheme, 'utf8', (err => {
                    if (err) {
                        console.log(err);
                        throw err;
                    }
